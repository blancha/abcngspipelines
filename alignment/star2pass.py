...
parser = argparse.ArgumentParser(description="Generates star second pass scripts.")
parser.add_argument("-s", "--scriptsDirectory", help="Scripts directory. DEFAULT=star2pass", default="star2pass")
...
parser.add_argument("-o", "--outputDirectory", help="Output directory with star results. DEFAULT=../results/star2pass", default="../results/star2pass")
...
limitSjdbInsertNsj = config.get("star", "limitSjdbInsertNsj")
...
for index, row in samplesFile.iterrows():
    ...
    # Create script file.
    scriptName = "star2pass_" + sample + ".sh"
    ...
    script.write(os.path.join(inputDirectory, file_R1) + " \\\n")
    script.write(os.path.join(inputDirectory, file_R2) + " \\\n")
    ...
    script.write("--outFileNamePrefix " + os.path.join(outputDirectory, sample, sample) + " \\\n")
    ...
    if not limitSjdbInsertNsj == "None":
        script.write("--limitSjdbInsertNsj " + limitSjdbInsertNsj + " \\\n")
    script.write("--sjdbFileChrStartEnd ")
    for sample in samples:
        script.write(os.path.relpath(os.path.join("../../results/star", sample, sample + "SJ.out.tab")) + " ")
    script.write("\\\n")
