...
parser.add_argument("-r", "--replicates", help="The condition will be determining by removing the last character from the replicate name.", choices=["yes", "no", "y", "n"], default="no")
...
if not singleEnd:
    ...
    for file1,file2 in util.pairwise(files):
        ...
        if args.replicates == "yes" or args.replicates == "y":
            condition = sampleName[:-1]
        else:
            condition = sampleName
        if lanes:
            lane = fields[2]
        if lanes:
            samples = samples.append({"file_r1": file1, "file_r2": file2, "lane": lane, "sample": sampleName, "condition": condition}, ignore_index=True)
        else:
            samples = samples.append({"file_r1": file1, "file_r2": file2, "sample": sampleName, "condition": condition}, ignore_index=True)

    # Sort samples dataframe by samples name
    samples = samples.sort_values(by="sample")
    # Convert samples and conditions columns to lower case
    samples["sample"] = samples["sample"].str.lower()
    samples["condition"] = samples.condition.str.lower()

    # Write samples file 
    if lanes:
        samples.to_csv("samples.txt", sep="\t", index=False, columns=["file_r1", "file_r2", "lane", "sample", "condition"])
    else:
        samples.to_csv("samples.txt", sep="\t", index=False, columns=["file_r1", "file_r2", "sample", "condition"])
...
if singleEnd:
    ...
    for file1 in files:
        ...
        if lanes:
            lane = fields[2]
        if lanes:
            samples = samples.append({"file_r1": file1, "lane": lane, "sample": sampleName, "condition": condition}, ignore_index=True)
        else:
            samples = samples.append({"file_r1": file1, "sample": sampleName, "condition": condition}, ignore_index=True)
    ...
    # Write samples file 
    if lanes:
        samples.to_csv("samples.txt", sep="\t", index=False, columns=["file_r1", "lane", "sample", "condition"])
    else:
        samples.to_csv("samples.txt", sep="\t", index=False, columns=["file_r1", "sample", "condition"])
